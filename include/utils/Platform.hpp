/**
 * @file utils/Platform.hpp
 * @brief Platform abstraction functions (signal handling, locking, threading)
 */

#ifndef WINDMI_UTILS_PLATFORM_HPP_
#define WINDMI_UTILS_PLATFORM_HPP_
...
public:
  ...
  template <typename Predicate> void wait(UniqueLock& lock, Predicate pred)
  {
    while (!pred())
      wait(lock);
  }
  ...
  template <typename Predicate> bool wait_for(UniqueLock& lock, unsigned int ms, Predicate pred)
  {
    while (!pred())
    {
      if (!wait_for(lock, ms))
...
#endif // WINDMI_UTILS_PLATFORM_HPP_
