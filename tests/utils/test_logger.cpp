...
TEST_F(LoggerTest, CBridgeLogsFormattedMessage)
{
  ...
  windmi_log(WINDMI_LOG_INFO, LOG_TAG_MODBUS, "test.c", 123, "func", "C bridge value %d", 17);
...
TEST_F(LoggerTest, ConcurrentLoggingDoesNotCrash)
{
  ...
  EXPECT_EQ(test_output_->entries.size(), static_cast<size_t>(num_threads * messages_per_thread));
}