...
{
  TEST(cmd_push_pop);
  spsc_cmd_t_queue_t q;
  memset(&q, 0, sizeof(q));
  cmd_t c = {.type = CMD_SET_DHW_TEMP, .float_val = 50.0f, .int_val = 0};
  if (!spsc_push_cmd_t(&q, c))
  {
    FAIL("push");
    return;
  }
  cmd_t o;
  if (!spsc_pop_cmd_t(&q, &o))
  {
    FAIL("pop");
    return;
  }
  if (o.type == CMD_SET_DHW_TEMP && o.float_val == 50.0f)
    PASS();
  else
    FAIL("value");
}
...
static void test_full(void)
{
  ...
  memset(&q, 0, sizeof(q));
  for (int i = 0; i < 15; i++)
    if (!spsc_push_cmd_t(&q, (cmd_t){CMD_SET_PRIORITY, 0, i}))
    {
      FAIL("premature");
      return;
    }
  if (!spsc_push_cmd_t(&q, (cmd_t){CMD_SET_DHW_TEMP, 0, 0}))
    PASS();
  else
    FAIL("");
}
...
static void test_fifo(void)
{
  ...
  cmd_t o;
  spsc_pop_cmd_t(&q, &o);
  bool a = (o.type == CMD_SET_DHW_TEMP && o.float_val == 1);
  spsc_pop_cmd_t(&q, &o);
  bool b = (o.type == CMD_SET_HEATING_TEMP && o.float_val == 2);
  spsc_pop_cmd_t(&q, &o);
  bool c = (o.type == CMD_SET_PRIORITY && o.int_val == 3);
  if (a && b && c)
    PASS();
  else
    FAIL("");
...
{
  TEST(status_latest);
  spsc_status_snapshot_t_queue_t q;
  memset(&q, 0, sizeof(q));
  ...
  status_snapshot_t l;
  bool f = spsc_latest_status_snapshot_t(&q, &l);
  if (f && l.outdoor_temp == 30.0f)
    PASS();
  else
    FAIL("");
}
