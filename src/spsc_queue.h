...
#ifndef WINDMI_SPSC_QUEUE_H_
#define WINDMI_SPSC_QUEUE_H_
...
typedef struct
{
  ...
  int running_status; // From REG_RUNNING_STATUS (0x002D): 0=Off, 1=Cool, 2=Heat, 4=DHW, 7=Defrost,
                      // 20=Anti-freeze
  bool dhw_priority;
  bool is_running;
  bool device_online;
...
static inline void spsc_push_status_snapshot_t(spsc_status_snapshot_t_queue_t* q,
                                               status_snapshot_t item)
{
...
static inline bool spsc_pop_status_snapshot_t(spsc_status_snapshot_t_queue_t* q,
                                              status_snapshot_t* item)
{
  uint32_t current_head = q->head;
...
static inline bool spsc_latest_status_snapshot_t(spsc_status_snapshot_t_queue_t* q,
                                                 status_snapshot_t* item)
{
  uint32_t current_tail = q->tail;
  uint32_t current_head = q->head;
