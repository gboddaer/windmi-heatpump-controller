...
void modbus_client_destroy(modbus_client_t *client) {
    if (client) {
        if (client->connected) modbus_client_disconnect(client);
#ifdef _WIN32
...
STATIC_FOR_TEST int receive_exact(modbus_client_t *client, uint8_t *buffer, size_t expected_len) {
    ...
    while (total_received < expected_len) {
        ...
        if (ready <= 0) {
            if (ready == 0) {
                WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Receive timeout (got %zu/%zu bytes)", total_received, expected_len);
            }
            return -1;
        }
        int received = (int)windmi_recv(client->socket_fd, buffer + total_received, expected_len - total_received, 0);
...
STATIC_FOR_TEST int modbus_read_write_registers_internal(modbus_client_t *client,
                                                         uint16_t address,
                                                         int16_t *values,
                                                         uint16_t count,
                                                         bool single) {
    ...
    if (header[1] & 0x80) {
        ...
        WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Modbus exception: slave=%d func=0x%02X code=%d", header[0], header[1], header[2]);
        return -1;
    }

    uint8_t byte_count = header[2];
    if (byte_count != read_count * 2) {
        WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Unexpected byte count: %d (expected %d)", byte_count, read_count * 2);
        return -1;
    }
    ...
    uint16_t crc_received = (uint16_t)full_frame[total_len - 2] | ((uint16_t)full_frame[total_len - 1] << 8);
...
int modbus_write_register(modbus_client_t *client, uint16_t address, uint16_t value) {
    ...
    for (int retry = 0; retry < MODBUS_WRITE_MAX_RETRIES; retry++) {
        if (retry > 0) {
            WINDMI_C_LOG(WINDMI_LOG_DEBUG, LOG_TAG_MODBUS, "Write retry %d/%d for address 0x%04X", retry, MODBUS_WRITE_MAX_RETRIES, address);
            windmi_sleep_ms(MODBUS_RETRY_DELAY_MS);
        }

        uint8_t frame[MODBUS_MAX_FRAME];
        modbus_rtu_build_write_frame(frame, client->slave_id, address, value);
        if (send_frame(client, frame, 8) != 0) continue;

        uint8_t response[8];
        if (receive_exact(client, response, 8) != 0) continue;

        if (response[1] & 0x80) {
            WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Modbus exception on write: slave=%d func=0x%02X exception_code=%d (attempt %d/%d)", response[0], response[1] & 0x7F, response[2], retry + 1, MODBUS_WRITE_MAX_RETRIES);
            continue;
        }
        if (response[0] != client->slave_id || response[1] != 0x06) {
            WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Write response mismatch: slave=%d func=0x%02X (attempt %d/%d)", response[0], response[1], retry + 1, MODBUS_WRITE_MAX_RETRIES);
            continue;
        }
        uint16_t crc_received = response[6] | (response[7] << 8);
        uint16_t crc_calculated = crc16(response, 6);
        if (crc_received != crc_calculated) {
            WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Modbus CRC error on write (attempt %d/%d)", retry + 1, MODBUS_WRITE_MAX_RETRIES);
            continue;
        }
        int16_t read_value;
        if (modbus_read_register(client, address, &read_value) != 0) continue;
        if ((uint16_t)read_value != value) {
            WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Modbus write verify failed: wrote %u, read %d (attempt %d/%d)", value, read_value, retry + 1, MODBUS_WRITE_MAX_RETRIES);
            continue;
        }
        if (retry > 0) {
...
int modbus_read_registers(modbus_client_t *client, uint16_t address, int16_t *values, uint16_t count) {
