...
bool acquire_instance_lock(bool force)
{
  ...
  if (fd < 0)
  {
    WINDMI_LOG_ERROR(LOG_TAG_PLATFORM, "Failed to open lock file %s: %s", lock_path.c_str(),
                     strerror(errno));
    return false;
  }
  ...
  if (flock(fd, LOCK_EX | LOCK_NB) < 0)
  {
    if (!force)
    {
      WINDMI_LOG_ERROR(LOG_TAG_PLATFORM, "Failed to acquire lock on %s: %s", lock_path.c_str(),
                       strerror(errno));
      close(fd);
      return false;
    }
...
bool SerialPort::open(const std::string& device, int baud, char parity, int stop_bits, bool rs485)
{
  ...
  if (!device.empty())
  {
    ...
    {
      wdev.resize(sz - 1);
      MultiByteToWideChar(CP_UTF8, 0, device.c_str(), -1, &wdev[0], sz);
    }
  }
  std::wstring norm = wdev;
  std::string sd = device;
  if (sd.size() >= 4 && ((sd[0] == 'C' || sd[0] == 'c') && (sd[1] == 'O' || sd[1] == 'o') &&
                         (sd[2] == 'M' || sd[2] == 'm')))
  {
    try
    {
      int pn = std::stoi(sd.substr(3));
      if (pn >= 10)
        norm = L"\\\\.\\COM" + std::to_wstring(pn);
    } catch (...)
    {}
  }
