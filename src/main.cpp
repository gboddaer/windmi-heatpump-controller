...
int main(int argc, char* argv[])
{
  ...
  for (int i = 1; i < argc; i++)
  {
    if (strcmp(argv[i], "--help") == 0 || strcmp(argv[i], "-h") == 0)
    {
      ...
      printf(
          "      --serial <device>   Serial device for Modbus RTU (e.g., /dev/ttyUSB0 or COM3)\n");
    ...
    } else if ((strcmp(argv[i], "--static-dir") == 0 || strcmp(argv[i], "-t") == 0 ||
                strcmp(argv[i], "--web-dir") == 0) &&
               i + 1 < argc)
    {
      static_dir = argv[++i];
    ...
    } else if (strcmp(argv[i], "--parity") == 0 && i + 1 < argc)
    {
      const char* p = argv[++i];
      if (strlen(p) != 1 ||
          (p[0] != 'N' && p[0] != 'E' && p[0] != 'O' && p[0] != 'n' && p[0] != 'e' && p[0] != 'o'))
      {
  ...
  } else if (!serial_device.empty())
  {
    ...
    WINDMI_LOG_INFO(LOG_TAG_MAIN, "Serial Modbus RTU: %s @ %d %c%d", serial_device.c_str(),
                    baud_rate, parity, stop_bits);
  } else
  {
  ...
  {
    g_web_server =
        new windmi::WebServer(web_port, resolved_static_dir, &cmd_queue, &status_queue, []() {
          if (g_control_loop)
          {
            g_control_loop->kick();
          }
  ...
  if (!demo_mode)
  {
    ...
    try
    {
      ...
      {
        shutdown_client =
            std::make_unique<windmi::ModbusClient>(modbus_ip, modbus_port, MODBUS_SLAVE_ID);
        WINDMI_LOG_INFO(LOG_TAG_MAIN, "Shutdown: using TCP client on %s:%d", modbus_ip,
                        modbus_port);
      }
