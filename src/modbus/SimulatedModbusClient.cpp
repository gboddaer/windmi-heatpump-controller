...
void SimulatedModbusClient::updateSimulationLocked() {
    ...
    if (elapsed < 0.1) return;  // Don't update too frequently
    ...
    if (running_mode == MODE_SET_OFF) {
        ...
        if (outdoor > ambient_temp_) {
            outdoor = (outdoor - static_cast<int16_t>(drift / 2) < ambient_temp_) ? ambient_temp_ : outdoor - static_cast<int16_t>(drift / 2);
        } else {
            outdoor = (outdoor + static_cast<int16_t>(drift / 2) > ambient_temp_) ? ambient_temp_ : outdoor + static_cast<int16_t>(drift / 2);
        }
