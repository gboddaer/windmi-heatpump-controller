...
static int serial_receive_exact(modbus_serial_client_t* client, uint8_t* buffer,
                                                    ...
                                                    uint8_t slave_id)
{
  ...
  WINDMI_C_LOG(
      WINDMI_LOG_INFO, LOG_TAG_MODBUS,
...
bool modbus_serial_client_connect(modbus_serial_client_t* client)
{
  ...
  client->port = windmi_serial_open(client->device, client->baud, client->parity, client->stop_bits,
                                    client->rs485_enabled);
  if (!client->port)
  {
    WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Failed to open serial device %s",
                 client->device);
    return false;
  }
  ...
  WINDMI_C_LOG(WINDMI_LOG_INFO, LOG_TAG_MODBUS, "Serial client connected: %s @ %d %d%c%d",
...
static int serial_send_frame(modbus_serial_client_t* client, const uint8_t* frame, size_t len)
{
  if (!client || !client->port || !frame || len == 0)
    return -1;
  windmi_serial_flush(client->port);
...
static int serial_receive_exact(modbus_serial_client_t* client, uint8_t* buffer,
                                size_t expected_len, int timeout_ms)
{
  if (!client || !client->port || !buffer || expected_len == 0)
    return -1;
  ...
  while (total_received < expected_len)
  {
    int received = windmi_serial_read(client->port, buffer + total_received,
                                      expected_len - total_received, (unsigned int)timeout_ms);
    if (received <= 0)
    {
      if (received == 0)
      {
                                                       ...
                                                       uint16_t count, bool single)
{
  ...
  if (serial_receive_exact(client, header, MODBUS_SERIAL_READ_HEADER_LEN,
                           MODBUS_SERIAL_RECV_TIMEOUT_MS) != 0)
  {
    client->connected = false;
    return -1;
  }
...
int modbus_serial_write_register(modbus_serial_client_t* client, uint16_t address, uint16_t value)
{
  ...
  for (int retry = 0; retry < MODBUS_SERIAL_WRITE_MAX_RETRIES; retry++)
  {
    if (retry > 0)
    {
      WINDMI_C_LOG(WINDMI_LOG_DEBUG, LOG_TAG_MODBUS, "Write retry %d/%d for address 0x%04X", retry,
                   MODBUS_SERIAL_WRITE_MAX_RETRIES, address);
    ...
    uint8_t response[8];
    if (serial_receive_exact(client, response, sizeof(response), MODBUS_SERIAL_RECV_TIMEOUT_MS) !=
        0)
    {
      client->connected = false;
      continue;
    ...
    if (response[0] != client->slave_id || response[1] != 0x06)
    {
      WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS,
                   ...
                   response[1], retry + 1, MODBUS_SERIAL_WRITE_MAX_RETRIES);
      continue;
    }
    ...
    if (crc_received != crc_calculated)
    {
      WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Modbus CRC error on write (attempt %d/%d)",
                   retry + 1, MODBUS_SERIAL_WRITE_MAX_RETRIES);
      continue;
    }
    ...
    if ((uint16_t)read_value != value)
    {
      WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS,
                   ...
                   read_value, retry + 1, MODBUS_SERIAL_WRITE_MAX_RETRIES);
      continue;
    }
  ...
  WINDMI_C_LOG(WINDMI_LOG_ERROR, LOG_TAG_MODBUS, "Write failed after %d attempts",
               MODBUS_SERIAL_WRITE_MAX_RETRIES);
  return -1;
}
